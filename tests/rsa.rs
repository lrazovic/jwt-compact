...
fn rs256_algorithm_with_generated_keys() {
	...
	let (signing_key, verifying_key) = Rsa::generate(&mut thread_rng(), ModulusBits::TwoKibibytes).unwrap();
...
fn ps256_checked_len_fails_on_undersized_key() {
	...
	let token = Rsa::ps256().token(&Header::empty(), &claims, &small_private_key).unwrap();
	...
	let err = StrongAlg(Rsa::ps256()).validator::<CompactClaims>(&public_key).validate(&token).unwrap_err();
...
fn test_rsa_reference(rsa: Rsa, token: &str) {
	...
	let validated_token = rsa.validator::<SampleClaims>(&public_key).validate(&token).unwrap();
	assert_eq!(validated_token.claims().issued_at.unwrap().timestamp(), 1_516_239_022);
	assert_eq!(
	...
	let public_key = StrongKey::try_from(public_key).unwrap();
	StrongAlg(rsa).validator::<SampleClaims>(&public_key).validate(&token).unwrap();
}

#[test]
fn rs256_reference() {
	// Generated using https://jwt.io/
	const TOKEN: &str = "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lI\
...
fn rs384_reference() {
	// Generated using https://jwt.io/
	const TOKEN: &str = "eyJhbGciOiJSUzM4NCIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lI\
...
fn rs512_reference() {
	// Generated using https://jwt.io/
	const TOKEN: &str = "eyJhbGciOiJSUzUxMiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lI\
...
fn ps256_reference() {
	// Generated using https://jwt.io/
	const TOKEN: &str = "eyJhbGciOiJQUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lI\
...
fn ps384_reference() {
	// Generated using https://jwt.io/
	const TOKEN: &str = "eyJhbGciOiJQUzM4NCIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lI\
