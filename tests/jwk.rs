...
fn hs256_jwk() {
	...
	assert_eq!(key_thumbprint::<Sha256, _>(&key), "y_x3gCJnL6oKGBBIXScabduwxTVy2Wd2bzRVEUbdUzc");
	assert_eq!(key_thumbprint::<Sha384, _>(&key), "fSRdJnLOn2xxz1WCXvwy2mlKp8SyEytzrcRUph9-YKryizmKOu9vfzKmM4HRRQYe");
	assert_eq!(
...
fn hs256_incorrect_key_type() {
	...
	assert_matches!(err, JwkError::UnexpectedKeyType { expected: KeyType::Symmetric, actual: KeyType::KeyPair });
}
...
mod rsa_jwk {
	...
	fn generate_multi_prime_key(rng: &mut impl Rng, nprimes: usize, bit_size: usize) -> RsaPrivateKey {
		assert!(nprimes > 2);
		...
		let key =
			RsaPrivateKey::from_components(n_final, exp, d_final, primes).expect("failed creating multi-prime key");
		key.validate().expect("generated multi-prime key is invalid");
		key
	}
	...
	fn verifying_jwk() {
		...
		assert_eq!(key_thumbprint::<Sha256, _>(&public_key), "NzbLsXh8uDCcd-6MNwXF4W_7noWXFZAfHkxZsRGC9Xs");
	...
	fn signing_jwk() {
		...
		assert_eq!(jwk.thumbprint::<Sha256>(), JsonWebKey::from(&public_key).thumbprint::<Sha256>());
	...
	fn incorrect_key_type() {
		...
		assert_matches!(err, JwkError::UnexpectedKeyType { expected: KeyType::Rsa, actual: KeyType::KeyPair });
	}
...
mod es256k {
	...
	fn verifying_jwk() {
		...
		assert_eq!(key_thumbprint::<Sha256, _>(&public_key), "WXjRM2dXofF2PGP339yJXhia89VsAQRBMZA5_lWuYFY");
		assert_eq!(
	...
	fn incorrect_key_type() {
		...
		assert_matches!(
			err,
			JwkError::UnexpectedKeyType { expected: KeyType::EllipticCurve, actual: KeyType::KeyPair }
		);
	}
...
mod es256 {
	...
	fn verifying_jwk() {
		...
		assert_eq!(key_thumbprint::<Sha256, _>(&public_key), "do-Rj_Jimba-KQ_4c8Ylb_kSAdv7YgOuWd7ug22Bbho");
		assert_eq!(
			key_thumbprint::<Sha384, _>(&public_key),
...
mod ed25519 {
	...
	#[test]
	fn verifying_jwk() {
		const KEY_BYTES: [u8; 32] = Hex.decode(b"b7e6ddbf8d4c2571315e7a6ab8706e0e7ee7d581b25fb80b41c8551c0a0dbb9d");
		...
		assert_eq!(PublicKey::try_from(&jwk).unwrap().as_bytes(), public_key.as_bytes());

		assert_eq!(key_thumbprint::<Sha256, _>(&public_key), "TZ72OrmiQl5Bz5Zm3NIM_0ksFtfP36SFdPZrwCj_2ZE");
		assert_eq!(
			key_thumbprint::<Sha384, _>(&public_key),
	...
	fn incorrect_key_type() {
		...
		assert_matches!(err, JwkError::UnexpectedKeyType { expected: KeyType::KeyPair, actual: KeyType::Symmetric });
	}
