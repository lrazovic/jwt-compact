...
fn hs256_reference() {
	...
	const TOKEN: &str = "eyJ0eXAiOiJKV1QiLA0KICJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJqb2UiLA0KICJleHAiOjEzMDA4MTkzODAs\
         ...
         gFWFOEjXk";
	const KEY: &str = "AyM1SysPpbyDfgZld3umj1qzKObwVMkoqQ-EstJQLr_T-1qS0gZH75aKtMN3Yj0iPS4hcgUuTwjAzZr1Z9CAow";
	...
	assert_eq!(validated_token.claims().expiration.unwrap().timestamp(), 1_300_819_380);
	assert_eq!(validated_token.claims().custom["iss"], json!("joe"));
	assert_eq!(validated_token.claims().custom["http://example.com/is_root"], json!(true));

	let checked_key = StrongKey::try_from(key).unwrap();
	#[allow(deprecated)] // Check that the deprecated API still works
	StrongAlg(Hs256).validate_integrity::<Obj>(&token, &checked_key).unwrap();
	StrongAlg(Hs256).validator::<Obj>(&checked_key).validate(&token).unwrap();
}
...
fn hs384_reference() {
	...
	let key = Hs384Key::from(KEY);
	#[allow(deprecated)] // Check that the deprecated API still works
	Hs384.validate_integrity::<SampleClaims>(&token, &key).unwrap();
	let token = Hs384.validator::<SampleClaims>(&key).validate(&token).unwrap();
	assert_eq!(token.claims().issued_at.unwrap().timestamp(), 1_516_239_022);
	assert_eq!(
		token.claims().custom,
		SampleClaims { subject: "1234567890".to_owned(), name: "John Doe".to_owned(), admin: true }
	);
}
...
fn hs512_reference() {
	...
	let token = Hs512.validator::<SampleClaims>(&key).validate(&token).unwrap();
	assert_eq!(token.claims().issued_at.unwrap().timestamp(), 1_516_239_122);
	assert_eq!(
		token.claims().custom,
		SampleClaims { subject: "987654321".to_owned(), name: "Jane Doe".to_owned(), admin: false }
	);
}
...
fn es256_reference() {
	...
	let token = Es256.validator::<Obj>(&public_key).validate(&token).unwrap();
	assert_eq!(token.claims().expiration.unwrap().timestamp(), 1_300_819_380);
...
fn es256k_reference() {
	...
	let token = es256k.validator::<Obj>(&public_key).validate(&token).unwrap();
...
fn ed25519_reference() {
	...
	const KEY: [u8; 32] = Hex.decode(b"06fac1f22240cffd637ead6647188429fafda9c9cb7eae43386ac17f61115075");
	...
	let token = Ed25519::with_specific_name().validator::<Obj>(&public_key).validate(&token).unwrap();
...
fn compact_token_hs256() {
	...
	#[cfg(feature = "std")]
	{
		...
		assert_matches::assert_matches!(generic_token.claims().custom["sub"], serde_cbor::Value::Bytes(_));
	}
}
...
fn high_s_in_signature_is_successfully_validated() {
	...
	let token = UntrustedToken::new(TOKEN).unwrap();
	<Es256k>::default().validator::<serde_json::Value>(&public_key).validate(&token).unwrap();
}
...
fn create_header() -> Header<HeaderExtensions> {
	Header::new(HeaderExtensions { custom: "custom".to_owned() }).with_token_type("JWT").with_key_id("test_key")
}
...
fn test_algorithm_with_custom_header<A: Algorithm>(
	...
	#[cfg(feature = "serde_cbor")]
	{
		let token_string = algorithm.compact_token(&header, &claims, signing_key).unwrap();
	...
	// Mutate header.
	let mangled_header = format!(r#"{{"alg":"{}","typ":"JWT","custom":"?"}}"#, algorithm.name());
	...
	let bogus_token_string = algorithm.token(&Header::empty(), &claims, signing_key).unwrap();
