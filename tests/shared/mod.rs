...
pub fn test_algorithm<A: Algorithm>(algorithm: &A, signing_key: &A::SigningKey, verifying_key: &A::VerifyingKey) {
	...
	{
		let token_string = algorithm.compact_token(&Header::empty(), &claims, signing_key).unwrap();
	...
	// Successful case.
	let token_string = algorithm.token(&Header::empty(), &claims, signing_key).unwrap();
	let token = UntrustedToken::new(&token_string).unwrap();
	let token = algorithm.validator(verifying_key).validate(&token).unwrap();
	assert_eq!(*token.claims(), claims);
	...
	let mangled_bits: Box<dyn Iterator<Item = usize>> = if signature_bits <= MAX_MANGLED_BITS {
		...
		let indexes = rand::seq::index::sample(&mut thread_rng(), signature_bits, MAX_MANGLED_BITS);
	...
	assert_ne!(claims_string, token_string[(header_end + 1)..(signature_start - 1)]);
	let mut mangled_str = token_string.clone();
