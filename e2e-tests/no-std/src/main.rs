...
impl TokenChecker {
	fn new() -> Self {
		Self { time_options: TimeOptions::new(Duration::seconds(15), now) }
	}

	fn extract_claims<'a>(&self, token: &'a Token<SampleClaims>) -> anyhow::Result<&'a SampleClaims> {
		Ok(&token.claims().validate_expiration(&self.time_options).map_err(|err| anyhow!(err))?.custom)
	}
	...
	fn verify_token<T: Algorithm>(
		...
		let token = alg.validator::<SampleClaims>(verifying_key).validate(&token).map_err(|err| anyhow!(err))?;
	...
	fn create_token<T: Algorithm>(
		...
		let claims = Claims::new(claims).set_duration_and_issuance(&self.time_options, Duration::minutes(10));

		let token = alg.token(&Header::empty(), &claims, signing_key).map_err(|err| anyhow!(err))?;
		Ok(token)
	}
		...
		T::VerifyingKey: VerifyingKey<T>,
	{
		...
		let claims =
			SampleClaims { subject: "j.doe@example.com".to_owned(), name: "John Doe".to_owned(), admin: false };
		...
		let verifying_key = <T::VerifyingKey>::from_slice(verifying_key).map_err(|err| anyhow!(err))?;
	...
	fn roundtrip_rsa(&self, alg: &Rsa, private_key_der: &[u8]) -> anyhow::Result<()> {
		...
		let claims =
			SampleClaims { subject: "j.doe@example.com".to_owned(), name: "John Doe".to_owned(), admin: false };
		let signing_key = RsaPrivateKey::from_pkcs1_der(private_key_der).map_err(|err| anyhow!(err))?;
		let token = self.create_token(alg, claims.clone(), &signing_key)?;
		hprintln!("Created token: {}", token);
