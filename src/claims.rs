...
#[derive(
	Debug, Clone, Copy, Default, PartialEq, Eq, Hash, Serialize, Deserialize, Encode, Decode, TypeInfo, MaxEncodedLen, DecodeWithMemTracking
)]
pub struct Empty {}
...
mod serde_timestamp {
	...
	impl<'de> Visitor<'de> for TimestampVisitor {
		...
		where
			E: DeError,
		{
			Utc.timestamp_opt(value, 0).single().ok_or_else(|| E::custom("UTC timestamp overflow"))
		}
		...
		where
			E: DeError,
		{
			Utc.timestamp_opt(value as i64, 0).single().ok_or_else(|| E::custom("UTC timestamp overflow"))
		}
	}
...
mod tests {
	...
	fn expired_claim() {
		...
		assert_matches!(claims.validate_expiration(&time_options).unwrap_err(), ValidationError::Expired);
		...
		assert_matches!(
			claims.validate_expiration(&TimeOptions::from_leeway(Duration::seconds(5))).unwrap_err(),
			ValidationError::Expired
		);
		// Same if we set the current time in the past.
		let expiration = claims.expiration.unwrap();
		assert!(claims.validate_expiration(&TimeOptions::new(Duration::seconds(3), move || { expiration })).is_ok());
	}
	...
	fn immature_claim() {
		...
		assert_matches!(claims.validate_maturity(&time_options).unwrap_err(), ValidationError::NotMature);
		...
		// If we set leeway lower, then the claim will be considered expired.
		assert_matches!(
			claims.validate_maturity(&TimeOptions::from_leeway(Duration::seconds(5))).unwrap_err(),
