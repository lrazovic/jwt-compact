...
impl AlgorithmSignature for Signature {
	const LENGTH: Option<NonZeroUsize> = NonZeroUsize::new(<FieldBytesSize<Secp256k1> as Add>::Output::USIZE);
	...
	VerifyingKey: DigestVerifier<D, Signature>,
{
	...
	fn verify_signature(
		...
		verifying_key.verify_digest(digest, &normalized_signature).is_ok()
	}
}
...
impl TryFrom<&JsonWebKey<'_>> for VerifyingKey {
	...
	fn try_from(jwk: &JsonWebKey<'_>) -> Result<Self, Self::Error> {
		...
		VerifyingKey::from_sec1_bytes(&key_bytes[..]).map_err(|err| JwkError::custom(anyhow::anyhow!(err)))
	}
}
...
impl TryFrom<&JsonWebKey<'_>> for SigningKey {
	...
	fn try_from(jwk: &JsonWebKey<'_>) -> Result<Self, Self::Error> {
		...
		let sk = Self::from_slice(sk_bytes).map_err(|err| JwkError::custom(anyhow::anyhow!(err)))?;
