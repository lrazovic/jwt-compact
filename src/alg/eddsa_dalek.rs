...
use ed25519_dalek::{
	SecretKey, Signature, Signer, Verifier, KEYPAIR_LENGTH, PUBLIC_KEY_LENGTH, SECRET_KEY_LENGTH, SIGNATURE_LENGTH,
};
...
impl VerifyingKey<Ed25519> for ed25519_dalek::VerifyingKey {
	fn from_slice(raw: &[u8]) -> anyhow::Result<Self> {
		let raw = <&[u8; PUBLIC_KEY_LENGTH]>::try_from(raw)
			.map_err(|err| anyhow::anyhow!(err).context("Ed25519 public key has unexpected length"))?;
...
impl<'a> From<&'a ed25519_dalek::VerifyingKey> for JsonWebKey<'a> {
	fn from(key: &'a ed25519_dalek::VerifyingKey) -> JsonWebKey<'a> {
		JsonWebKey::KeyPair { curve: Cow::Borrowed("Ed25519"), x: Cow::Borrowed(key.as_ref()), secret: None }
	}
}
