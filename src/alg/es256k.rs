...
use secp256k1::{
	constants::{COMPACT_SIGNATURE_SIZE, FIELD_SIZE, SECRET_KEY_SIZE, UNCOMPRESSED_PUBLIC_KEY_SIZE},
	ecdsa::Signature,
	All, Message, PublicKey, Secp256k1, SecretKey,
};
use sha2::{
	digest::{crypto_common::BlockSizeUser, generic_array::typenum::U32, FixedOutputReset, HashMarker},
	Digest, Sha256,
};
...
{
	fn default() -> Self {
		Es256k { context: Secp256k1::new(), _digest: PhantomData }
	}
}
	...
	D: FixedOutputReset<OutputSize = U32> + BlockSizeUser + Clone + Default + HashMarker,
{
	...
	pub fn new(context: Secp256k1<All>) -> Self {
		Es256k { context, _digest: PhantomData }
	}
}
	...
	D: FixedOutputReset<OutputSize = U32> + BlockSizeUser + Clone + Default + HashMarker,
{
	...
	fn verify_signature(
		...
		self.context.verify_ecdsa(&message, &normalized_signature, verifying_key).is_ok()
	}
}
