...
impl Algorithm for Rsa {
	...
	fn sign(&self, signing_key: &Self::SigningKey, message: &[u8]) -> Self::Signature {
		...
		let signing_result = match self.padding_scheme() {
			PaddingScheme::Pkcs1v15(padding) => signing_key.sign_with_rng(&mut rand_core::OsRng, padding, digest),
			PaddingScheme::Pss(padding) => signing_key.sign_with_rng(&mut rand_core::OsRng, padding, digest),
		};
...
impl TryFrom<&JsonWebKey<'_>> for RsaPublicKey {
	...
	fn try_from(jwk: &JsonWebKey<'_>) -> Result<Self, Self::Error> {
		let JsonWebKey::Rsa { modulus, public_exponent, .. } = jwk else {
...
impl TryFrom<&JsonWebKey<'_>> for RsaPrivateKey {
	...
	fn try_from(jwk: &JsonWebKey<'_>) -> Result<Self, Self::Error> {
		let JsonWebKey::Rsa { modulus, public_exponent, private_parts } = jwk else {
			return Err(JwkError::key_type(jwk, KeyType::Rsa));
		};

		let RsaPrivateParts { private_exponent: d, prime_factor_p, prime_factor_q, other_prime_factors, .. } =
			private_parts.as_ref().ok_or_else(|| JwkError::NoField("d".into()))?;
		...
		factors.extend(other_prime_factors.iter().map(|prime| BigUint::from_bytes_be(&prime.factor)));

		let key = Self::from_components(n, e, d, factors);
		let key = key.map_err(|err| JwkError::custom(anyhow::anyhow!(err)))?;
		key.validate().map_err(|err| JwkError::custom(anyhow::anyhow!(err)))?;
		Ok(key)
	}
}
