...
pub trait Algorithm {
	...
	fn verify_signature(&self, signature: &Self::Signature, verifying_key: &Self::VerifyingKey, message: &[u8])
		-> bool;
}
...
impl<A: Algorithm> Algorithm for Renamed<A> {
	...
	fn verify_signature(
		...
		self.inner.verify_signature(signature, verifying_key, message)
	}
}
...
impl<A: Algorithm> AlgorithmExt for A {
	...
	where
		T: Serialize,
	{
		let complete_header = CompleteHeader { algorithm: self.name(), content_type: None, inner: header };
	...
	{
		let complete_header =
			CompleteHeader { algorithm: self.name(), content_type: Some("CBOR".to_owned()), inner: header };
		let header = serde_json::to_string(&complete_header).map_err(CreationError::Header)?;
		let mut buffer = Vec::new();
		encode_base64_buf(&header, &mut buffer);
	...
	fn validator<'a, T>(&'a self, verifying_key: &'a Self::VerifyingKey) -> Validator<'a, Self, T> {
		Validator { algorithm: self, verifying_key, _claims: PhantomData }
	}
	...
	where
		T: DeserializeOwned,
	{
		self.validator::<T>(verifying_key).validate_for_signed_token(token)
	}
}
...
impl<A: Algorithm + ?Sized, T: DeserializeOwned> Validator<'_, A, T> {
	...
	pub fn validate<H: Clone>(self, token: &UntrustedToken<H>) -> Result<Token<T, H>, ValidationError> {
		self.validate_for_signed_token(token).map(|signed| signed.token)
	}
	...
	pub fn validate_for_signed_token<H: Clone>(
		...
		let signature = A::Signature::try_from_slice(signature).map_err(ValidationError::MalformedSignature)?;
		// We assume that parsing claims is less computationally demanding than
		// validating a signature.
		let claims = token.deserialize_claims_unchecked::<T>()?;
		if !self.algorithm.verify_signature(&signature, self.verifying_key, &token.signed_data) {
			return Err(ValidationError::InvalidSignature);
		}

		Ok(SignedToken { signature, token: Token::new(token.header().clone(), claims) })
	}
}
