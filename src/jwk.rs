	...
	fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			Self::UnexpectedKeyType { expected, actual } => {
				write!(formatter, "unexpected key type: {actual} (expected {expected})")
			},
			Self::NoField(field) => write!(formatter, "field `{field}` is absent from JWK"),
			Self::UnexpectedValue { field, expected, actual } => {
				write!(formatter, "field `{field}` has unexpected value (expected: {expected}, got: {actual})")
			},
			Self::UnexpectedLen { field, expected, actual } => {
				write!(formatter, "field `{field}` has unexpected length (expected: {expected}, got: {actual})")
			},
			Self::MismatchedKeys => formatter.write_str("private and public keys encoded in JWK do not match"),
...
impl JsonWebKey<'_> {
	...
	pub fn to_verifying_key(&self) -> Self {
		match self {
			...
			Self::KeyPair { curve, x, .. } => Self::KeyPair { curve: curve.clone(), x: x.clone(), secret: None },
		}
	}
	...
	pub fn thumbprint<D: Digest>(&self) -> Output<D> {
		let hashed_key = if self.is_signing_key() { Cow::Owned(self.to_verifying_key()) } else { Cow::Borrowed(self) };
...
mod helpers {
	...
	impl JsonWebKey<'_> {
		...
		pub(crate) fn ensure_len(field: &str, bytes: &[u8], expected_len: usize) -> Result<(), JwkError> {
			if bytes.len() == expected_len {
				Ok(())
			} else {
				Err(JwkError::UnexpectedLen { field: field.to_owned(), expected: expected_len, actual: bytes.len() })
			}
		}
		...
		D: Deserializer<'de>,
	{
		...
		impl Visitor<'_> for Base64Visitor {
			...
			fn visit_str<E: DeError>(self, value: &str) -> Result<Self::Value, E> {
				Base64UrlUnpadded::decode_vec(value).map_err(|_| E::invalid_value(Unexpected::Str(value), &self))
			}
...
mod tests {
	...
	fn serializing_jwk() {
		...
		assert_eq!(json, serde_json::json!({ "crv": "Ed25519", "kty": "OKP", "x": "dGVzdA" }));
	...
	fn jwk_deserialization_errors() {
		...
		let missing_field_err = serde_json::from_str::<JsonWebKey<'_>>(missing_field_json).unwrap_err().to_string();
		assert!(missing_field_err.contains("missing field `kty`"), "{missing_field_err}");

		let base64_json = r#"{"crv":"Ed25519","kty":"OKP","x":"??"}"#;
		let base64_err = serde_json::from_str::<JsonWebKey<'_>>(base64_json).unwrap_err().to_string();
		assert!(base64_err.contains("invalid value: string \"??\""), "{base64_err}");
		assert!(base64_err.contains("base64url-encoded data"), "{base64_err}");
	}
