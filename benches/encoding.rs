...
fn decoding_benches(criterion: &mut Criterion) {
	...
	#[cfg(feature = "serde_cbor")]
	{
		...
		criterion
			.bench_function("decoding_cbor", |bencher| bencher.iter(|| UntrustedToken::new(&compact_token).unwrap()));
		criterion.bench_function("decoding_cbor/full", |bencher| {
			bencher.iter(|| {
				let token = UntrustedToken::new(&compact_token).unwrap();
				Hs256.validator::<CustomClaims>(&key).validate(&token).unwrap()
			});
		});
	}

	let token = Hs256.token(&header, &claims, &key).unwrap();
	criterion.bench_function("decoding", |bencher| bencher.iter(|| UntrustedToken::new(&token).unwrap()));
